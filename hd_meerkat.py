...
if dec > 9:
	...
	icon = str(dec)
print "${voffset 4}${color0}${font Pie charts for maps:size=14}"+icon+"${font}${color}   ${voffset -5}Root: ${color1}${fs_free_perc /}%${color} ${alignr}${color2}${fs_free /}${color}"
...
if os.path.ismount("/home"):
	...
	print "${voffset 4}${color0}${font Pie charts for maps:size=14}"+icon+"${font}${color}   ${voffset -5}Home: ${color1}${fs_free_perc /home}%${color} ${alignr}${color2}${fs_free /home}${color}"
...
for device in os.listdir("/media/"):
	if (not device.startswith("cdrom")) and (os.path.ismount('/media/'+device)):
		...
		if dec > 9:
			...
			icon = str(dec)
		# end calculation dec
		print "${voffset 4}${color0}${font Pie charts for maps:size=14}"+icon+"${font}${color}   ${voffset -5}"+device.capitalize()+": ${color1}${fs_free_perc /media/"+device+"}%${color} ${alignr}${color2}${fs_free /media/"+device+"}${color}"
